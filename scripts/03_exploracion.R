...
library(naniar)    # Para la visualización de datos faltantes 
library(patchwork) # Para combinar gráficos
#Definimos solo en este las librerias por que no se usan en todos los scripts
...
# 1. ESTRUCTURA GENERAL, DIMENSIONES Y TIPOS DE DATOS
...
glimpse(df_prod_export_limpio) # Identificación de columnas y tipos de datos 
...
# verificamos que las variables clave sean numéricas y que seccion sea categórica para el ANOVA.





# 2. DATOS FALTANTES (NAs) 

mensaje_proceso("2. Cuantificación y patrón de datos faltantes (NAs)")

# Cuantificación de NAs por columna 
...
if (nrow(reporte_na_base_export) > 0) {
  ...
  # 2. Guardar el reporte detallado como CSV para consultarlo despues
...
#¿Qué porcentaje de cada sección son NAs?
...
# Utilizamps una función creeada en el globar para estadisticas descriptivas
...
# buscamos Si Media > Mediana, para considerar la transformación logarítmica 

# 3.2 Base 2: Variables Continuas (Centralidad para Hipótesis de anova)
...
# Histograma para ver forma/asimetría y Boxplot para ver outliers IQR
...
# Boxplot de Potencialidad (Detección formal de outliers)
...
# 4.2 Base 2: Boxplots de Centralidad por Sección (Visualización para ANOVA)
# Se comprueba visualmente si la centralidad media difiere entre sectores
p_centralidad_seccion <- df_prod_export_limpio %>%
  ...
  geom_boxplot(alpha = 0.7, outlier.shape = NA) + # Oculta outliers para usar jitter
...
# 5. PRIMERAS OBSERVACIONES Y ANOMALÍAS
...
# importante en /output/tables/ para el informe final
